...
List<RelationshipMemory> buildRelationshipMemories({
  ...
  for (final m in messages) {
    ...
    // ⚠️ Two prefixes, and both have to stay. Strips have been saved as
    // 'strip-' since the booth shipped, so that is what is actually in the
    // bucket; 'booth-' is what [PhotoOrigin] writes. Dropping either one
    // reclassifies real strips as ordinary day photos. The template-name
    // check is the last resort for strips saved before either prefix.
    final booth = m.isPhoto &&
        (m.imagePath!.split('/').last.startsWith('strip-') ||

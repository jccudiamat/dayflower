...
- **8.4 MB of PNG became 752 KB**: trimmed to the artwork's own bounding box, longest side to 760px (they are drawn at about 350pt on a phone), webp quality 86 with alpha. Originals kept in `twolip/design/frames/`.
- 🔴 **The art is a cut-out with a hole in it, not a border.** A photo goes *behind* it and shows through, so every frame carries `windows` as fractions of its box. ⚠️ **The fractions were measured from the alpha channel** (flood the outside, and the interior transparent regions are the windows) rather than typed by hand, so they cannot drift from the picture they describe. Five frames take one photo, five take two, and `torn_note` takes **none**: it is a sheet to write on, and a window there would put a photo where the words go.
- ⚠️ Three of the fourteen supplied images are **contact sheets** of nine frames each, not cut-outs, at roughly 512px per frame. They are in `design/frames/` as `sheet_*.png` and are deliberately **not** assets: they hold designs the eleven do not (a navy night frame, a film strip, daisies, purple pairs), but at a third of the resolution and needing slicing. Worth cutting properly if any are wanted.
- `frameById` returns **null** for an unknown id rather than falling back: a message naming a frame this build lacks came from a newer one, and quietly drawing a different frame would misrepresent what was sent.
- Tests: photo_frames_test.dart checks every asset is bundled and decodes at its declared aspect, every window sits inside its frame and is big enough to be one, ids are unique, and the torn note has no window. 583 passing.
**The camera now uses them.**

- **The row beside the shutter is the frames**, scrolling horizontally, where a camera app puts its filters. The shutter moved left to make room. Only the **one-photo** frames are offered there, because one press of the shutter is one photo; the rest are on the Templates page where there is room to say what they need.
- **A CAMERA / TEMPLATES row at the bottom**, faded and scrollable like Instagram's, with **space under it**: it used to sit against the gesture bar, where a swipe up was as likely to leave the app as to press anything.
- 🔴 **The photo-booth strips moved out of the camera** onto the new `TemplatesScreen` (`Routes.templates`). They were the camera's only row, so every new frame would have made it longer until it was the whole screen. The page groups One photo / Two photos / Paper / Photo booth, has a search, and hands a `TemplateChoice` back down to the camera. A chosen strip shows as a pill above the controls, since it has no dot to light.
- **Choosing a frame bakes it into the shot** (`FrameCompositor`): the photo is drawn behind, the paper on top, and the result is sent as an ordinary day photo. ⚠️ Baked rather than layered, because the widget, the thread and a copy saved to the gallery all have to be the same image. PNG, not JPEG: a torn note is not a rectangle and JPEG would fill its edges with black. A compositor that fails sends the bare photo, since the picture is the point and the paper is decoration.
...
- 🔴 **Stopping the recorder no longer sends.** It was the one message in the app you could not skim before it left, and the only one you could not say anything alongside. The recording now waits on the composer as an attachment, exactly as a picked photo does: `VoicePreview` (play, scrub, the length counting down, a bin), the ordinary field becomes "Add a caption", and the same Send button posts it. A failed send puts the recording back rather than losing what was said.
  ...
  - ⚠️ A pending recording greys the photo, camera and mic buttons: a picture and a voice in one bubble is a thing the thread cannot draw.
  - Discarding deletes the file. It lives in the cache directory, so a missed delete is untidy rather than a leak, but every abandoned take would otherwise stay there.
- **The visualiser is rounded bars with a dotted tail** (`voice_wave.dart`, `VoiceBarsPainter`), to a reference the user supplied. ⚠️ Bars, not the filled curve built first: a curve reads as a level meter and hides how long the recording is, while bars are a count of moments as well as a picture of loudness, and the dotted run-on says there is room left. One bar per ~0.3s, each the **peak** of its slice (an average flattens speech to a straight line), scrolling once the row is full. A finished note draws a stable shape from `waveFor(id)` with the played bars in brand pink. ⚠️ Still not the real sound, and the header says why: decoding every note in a thread to draw it is a lot of bytes and battery for decoration, and the length and progress — the honest parts — are real.
- **Recording pauses and carries on** (`MediaRecorder.pause/resume`, fine at the app's minSdk of 24). 🔴 The elapsed time comes back **from the recorder**, not from the Dart timer: a paused recorder writes nothing, so a timer that kept counting would promise seconds the file does not contain and `audio_ms` would disagree with the audio. `VoiceNotes.kt` banks each run in `bankedMs` and restarts the clock on resume. The bars grey out while paused, so the row looks stopped rather than merely still.
- **`_SendButton` had no semantics label at all**: a screen reader announced the control that sends the message as nothing. Fixed.
- 🔴 **`typingLineProvider` threw on the first keystroke** where the Supabase client was not up, from inside `onChanged`, taking the composer down. Guarded: an ellipsis is never worth that.
- Tests: voice_preview_test.dart (stopping does not send, the caption goes with it, play back, discard, pause freezes the clock and resumes, the other attachers stand down). ⚠️ The fake recorder writes a **real** file and the send is driven inside `runAsync`: the send path reads the recording off disk, and file I/O never completes on a widget test's fake clock. 579 passing.
...
- **Peer calls** (`peer_call_transport.dart`, migration **0050**). Every room holds exactly two people by construction, so the SFU was ~90% of the modelled cost for a job the app never needs. Same `CallTransport` interface, so this is one new file rather than a rewrite.
  ...
  - Whoever has the **lexicographically smaller user id offers**, derived on both phones, so no round trip and no glare.
  - **Relay is Cloudflare**, via the new `turn` edge function (deploy with `--verify-jwt`: the caller is a phone, so Supabase checking the token *is* the access control). Secrets `CF_TURN_KEY_ID` + `CF_TURN_API_TOKEN`; a self-hosted coturn (`TURN_URLS` + `TURN_SECRET`) is the alternative path in the same function. 🔴 **Not in SQL**, unlike `livekit_token`: Cloudflare mints credentials over HTTP and Postgres only has `pg_net`, which is queued and cannot answer a waiting request.
  - Cost: **$0.05/GB, first 1,000 GB free**, and only relayed calls count. ~$176/mo at 5,000 pairs and 30 min/day against ~$5,009 on LiveKit. Per-allocation limits (50-100 Mbps, 5-10 kpps) are far above a two-person call.
  - 🔴 `.env` ships **`CALL_TRANSPORT=livekit`** until the Cloudflare secrets are set; the code's own default is `peer`.
  - `call_video.dart` renders from either transport (`_PeerVideo` owns an `RTCVideoRenderer`, which is a native texture and so must be stateful). `flutter_webrtc` is a direct dependency now, already in the build via livekit_client, so no APK growth.
- **Typing indicator** at the **foot of the conversation** (`typing_bubble.dart`, `TypingFooter`), not the header: the header says who you are talking to, this is a thing happening in the thread. Three dots that roll, and `AnimatedSize` so the last message does not jump. Also the Chats row preview. 🔴 One message per burst, not per keystroke: "started", repeated every 4s only while still typing, "stopped" on send or an emptied field, and the receiver forgets after 7s so a lost "stopped" cannot strand it.
- **Voice messages, 2 minutes** (migration **0051**, `VoiceNotes.kt`, `voice_notes.dart`, `voice_bubble.dart`, `voice_recorder_bar.dart`). Columns `audio_path` + `audio_ms` on `flower_messages`, private `voice_notes` bucket keyed `<pair_id>/…`, `delete_message` returns whichever object the row owned and the app routes by `.m4a`.
  - **Android MediaRecorder/MediaPlayer over a channel, not a package**: recorder packages carry their own audio engine and the APK has ~76 KB of headroom. **24 kbps mono at 16 kHz**: two minutes is ~360 KB against ~1.8 MB at a package's usual default.
  - Tap to start, tap to send, visible cancel — not hold-to-talk. A two-minute hold is a bad gesture and an inaccessible one.
  - `audio_ms` on the row so the bubble draws at its real width before anything downloads. Waveform bars are a fixed shape from the message id: a real one would mean downloading every note in the thread to render it.
...
- **Bottom nav**: "Dayflower" is **Create**, "Chat" is **Chats** (app_bottom_nav.dart). The Create page's own title says Create too. The garden section is gone from Create (Memories → Flowers has it), and **Blooms now lights the Memories tab** (`sectionForLocation`), since Memories is the only way in.
- **Mood notifications** (migration **0049_mood_shared.sql**, trigger `users_mood_shared` on `users.mood, mood_at`): choosing a mood, changing it, or choosing the same one on a new day (the setter's day, as `freshMood` judges it) logs a `mood_set` activity ("Feeling loved", 🥰; `ActivityKind.moodSet`, route Home, "You shared how you feel" for your own) and posts a `mood` push. A change within ten minutes **updates** the last mood activity instead of adding one (an update, because this table's deletes do not reach the other phone over realtime). Clearing a mood is not news. Push function: `mood` kind, body "Feeling loved 🥰", TTL 12 hours. Phone: `PushKind.mood` shows on the Activity channel under the activity id, the same id and wording the realtime path uses (`_alertActivity` special-cases moods), so the two replace each other rather than stack.
  - **Shipped 2026-09-25 in that order: `push` deployed (version 6), then 0049 applied** and the trigger read back (enabled, security definer). The order matters for any future change: an older function describes an unknown kind as "sent you a message". Validated beforehand in a rolled-back transaction: two changes gave one activity and two queued pushes.
- **Ring light** (ring_light.dart): now one blurred frame (everything outside a rounded rectangle just inside the screen, blurred once), drawn as a cached image per size and width, instead of concentric strokes. Lit into every corner (the old rounded rim left dark wedges on tighter-cornered phones), no seams. While it is on, the **navigation bar takes the light's colour** with dark buttons (AnnotatedRegion in `_LiveView`), so the light reaches the bottom edge on phones where the app does not draw under the bar. **Width slider** (16 to 110, default 40, `ringLightWidthProvider`, saved on release) sits above the controls while the light is on and hides with them.
- Tests: ring_light_test (corners as lit as the sides, width reaches further, clamp), activity_test and push_message_test (mood), nav tests renamed; Blooms ownership moved. 565 passing. **Published as build 111** (APK 52,351,002 bytes, about 76 KB under the cap).
...
- **Settings → More → Bugs and suggestions** (`lib/features/feedback/`, `Routes.feedback`): Bug or Suggestion, text (up to 4000), up to three screenshots from the gallery, Send. Sends app version and OS version with it. On failure everything stays for a retry; uploaded screenshots are removed again.
  - **Migration 0048_feedback.sql applied 2026-09-25** and read back: table `feedback` with RLS (insert and read own only), bucket `feedback` private, five policies, `authenticated` can insert. Screenshots keyed `<user_id>/…`. Reports are read in the dashboard (Table editor → feedback; screenshots in Storage → feedback).
- Tests: test/chat_search_test.dart (arrow, count, four tiles, Message focus, search to jump, helpers), test/feedback_test.dart (send, limit of three, failure keeps the draft). Screenshots: `--dart-define=CAPTURE_REVIEW=true` → build/review/chat-arrow*.png, chat-settings-actions.png, chat-search*.png, feedback.png. Full suite 560 passing. **Published as build 110.** 🔴 APK now 52,350,906 bytes: **about 76 KB under the 50 MiB cap** (was about 205 KB). Anything with weight (art, fonts, a library) needs space freed first.
...
- **Preview copy is first person: "…, I made this for you" / "Open when you have a minute."** It sits directly under the sender's own message in a chat, so it should read as them speaking, not as the site announcing. ⚠️ The *email* subject is still third person ("Jc left you something") — that one comes from Dayflower, not from the sender, so "I" would be wrong there.
- **Single stem shows one flower and nothing else** — the ribbon is gone — and its bloom is picked independently of the bouquet via `stemFlower`, swiped or arrowed through all 54. Falls back to the bouquet's first flower so it is never empty. Arrows exist because a swipe is invisible and unreachable by keyboard.

- **Send by email, 2026-09-13 — migration 0040 applied.** An address field in the Send step posts to `/api/gift/email`, which loads the gift, claims a rate-limit slot, and hands it to Resend. Template is `website/emails/bouquet-gift.ts`, same table-and-inline-styles shape as the waitlist confirmation.
- 🔴 **`/api/gift/email` is an open mail relay if the limiter ever comes off.** `claim_gift_email()` enforces **5 per sender per hour** and **3 per gift per day** in one statement, so two racing requests cannot both pass. Without it, anyone could have our domain deliver to any address — which gets the Resend account suspended and takes the waitlist confirmation down with it. **Verified:** the per-gift ceiling refuses the 4th send.
- **The recipient's address is never stored.** `gift_email_sends` keeps only the gift id and a salted SHA-256 of the sender's IP (`RATE_LIMIT_SALT`, falling back to a constant). The table has *no* RLS policies at all, so it is reachable only through the definer function.
- ⚠️ **`from` is always our own domain.** The sender's name goes in the body, never the From header — letting a stranger put any name and address on mail we send is how this becomes a phishing tool.
- ⚠️ **The subject gives nothing away** ("Jc left you something 🌷"), for the same reason the link preview doesn't: it lands on a lock screen before it is opened. The one image is the gift's own OG card, so a client that blocks images still gets a complete email from the text.
- Preview either template in dev at `/api/email-preview?template=gift` (or no query for the waitlist one). 404s in production.
- **"Send in the Dayflower app"** copies the link and opens `dayflower://gift/<id>`.
  - 🔴 **The app cannot receive it yet.** Every `dayflower://` URI in `lib/` is a *notification payload or home-widget action*; there is no `app_links`/`uni_links` dependency, so nothing reads an inbound URI from a browser. The manifest's intent filter means Android will launch the app, but it opens wherever it normally does. Until a receiver exists the button is copy-and-paste, and the notice says so.
  - Making it land in the chat needs: the `app_links` package, a handler routing `gift/<id>`, a `flower_messages` row, and an app build. The website has no session and no idea who anyone's partner is, so it can never post that row itself.

- **Shareable gift links land at `/g/<id>`, 2026-09-13 — migration 0039 applied.** Sending it produces a 36-character link instead of a 1000+ character one, and each gift gets its own preview card in chat and mail.
- 🔴 **Why a stored row was unavoidable.** A `#gift=` fragment is *never sent to the server*, so the crawler behind a link preview only ever saw the generic `/bouquet` page — every gift showed an identical card, which is exactly what the user reported. A preview must be server-rendered, and the server can only render what it can read. No amount of template work fixes that.
- ⚠️ **Nothing in a preview may name what is inside.** `/g/<id>`'s title, description and OG image say "someone left this for you" and never *bouquet* or *flowers* — the recipient sees them in a notification before opening. `/bouquet`'s `<title>` and meta description stay explicit for search; only its **openGraph** block was made discreet, because that is what lands in a chat. Keep that split.
- The card shows the **sealed vessel, never the bouquet**, which is the reason the vessel is a separate choice from the wrapping paper.
- ⚠️ `opengraph-image.tsx` draws the vessels **again, in SVG**, because `ImageResponse` renders through Satori and Satori has no canvas — `renderVessel` cannot be reused. They are deliberately crude silhouettes. When the vessels get real artwork this becomes one `<img>` per vessel and the shapes go away.
- **Photo bouquets are shareable.** That restriction only ever existed because a data URL will not fit in a URL fragment; a stored row has no such limit. **Verified on production:** a single-photo gift stores and renders, and so does the editor's worst case — eight photos at `MAX_PHOTO_LENGTH` each, 1.44 MB. `encodeBouquet` still refuses photos, but it is only reachable from the *private* link now, and its message says so.
  - ⚠️ The Photos step told people the opposite for a day ("not included in the gift link"), which is what it looked like from the outside. When a limit is lifted, grep the copy: the note in the Photos panel and the emphasis on Download were both written for the old behaviour.
- The `#gift=` link survives as a **private link**, tucked behind a disclosure in the Send step: nothing stored, no preview card, no photos. That keeps the old promise available to anyone who wants it.
- **RLS does the real work.** Insert is open, select is by id *and* `expires_at > now()`, and there is no update or delete for anyone — a gift is immutable once sent, which is what makes "the original link keeps its original flowers" true rather than aspirational. `open_bouquet_gift()` is a definer function so the one mutable column cannot be forged, the same shape as `end_call()` in 0025. **Verified:** missing, malformed and expired ids all 404, and the expiry is enforced by the policy rather than by app code.
- Viewing a gift never touches the viewer's own saved draft — checked by planting a draft, opening someone else's gift, and reading it back.
- ⚠️ **Privacy page and the bouquet FAQ were updated in the same commit.** `/bouquet` previously promised that nothing is stored; that is now true only of the private link. Keep those three in step if the storage model changes again.

- **Everything is grabbable, 2026-09-12.** Flowers and photos each carry a tilt dial and a size grip on the canvas; every slider they duplicate stays. Grips are drawn *outside* the object's own transform, so they keep one size however far a photo has been scaled. A stem's two grips are mirrored across the bloom so they can never overlap; a photo's ride its top-right and bottom-right corners and turn with it.
- ⚠️ **Scale drags read the ratio of distances from the object's own origin**, not raw pointer delta, so the grip stays under the finger wherever it was grabbed. A consequence worth knowing when testing: dragging *along* a circle around that origin correctly changes nothing. Both scale and tilt clamp to the same bounds `validateBouquet` enforces.
- **Flowers and photos share one stack** (`layeredItems`, `reorder`). Send back / Bring forward moves one step through the combined list and renumbers `z` to 0..n-1. The old `layer: back | front` on photos is kept only so pre-existing links still decode — it is no longer consulted once an item has a real `z`.
- 🔒 **Nothing can be reordered out of the wrapper.** The whole layered loop sits between `drawWrapper(false)` and `drawWrapper(true)`, so depth shuffles the contents and the paper always closes over them. That is structural, not a rule anyone has to remember — **verified** by pushing a photo to the top of the stack and sampling canvas pixels over the wrapper body, which still came back as paper.
- **The wrapper scales and leans** (`wrapScale` .75–1.3, `wrapAngle` ±20°) about its tie at `WRAP_PIVOT`, not its centre, so it leans the way a held bouquet does. Sliders rather than canvas grips: the wrapper has no selection model and handles would fight the flowers sitting on top of it.
  - ⚠️ The taper clip is laid down **under the wrapper transform, then the transform is undone before `clip()`**. The current path is not part of the state `save`/`restore` touches, so it survives with its points already resolved — which is what lets the clip follow the tilted paper while the flowers inside draw untransformed. Do not "simplify" this by moving the `clip()` inside the `save`.
- `z`, `wrapScale` and `wrapAngle` are defaulted rather than rejected, like `vessel`/`print`, and omitted from the payload at their defaults. `layeredItems` falls back to exactly the old hardcoded order (photos-back, stems, photos-front), so a link sent before any of this renders unchanged.

- **Vessels + stationery, 2026-09-12.** Six vessels (envelope · postcard · love letter · carrier pigeon · single stem · gift box) and nine prints (5 tiled patterns, 3 borders, plus Plain) with an opacity slider. The vessel is what the recipient meets *before* anything opens, which is why it is separate from the paper the flowers are wrapped in — it is also what every future preview card will show.
- **Both are drawn in code** (`renderVessel`, `drawPrint`), deliberately, so the send flow did not wait on artwork. Swapping in a sprite sheet later means rewriting only those function bodies; the stored indexes stay valid. "Single stem" is the exception — it borrows the bouquet's own first flower from the real sheets.
- ⚠️ **`vessel`, `print` and `printOpacity` are defaulted in `validateBouquet`, not rejected** — unlike every other field. Links sent before these existed carry none of them, and a link someone already posted must never stop opening because the editor grew a feature. Out-of-range values fall back for the same reason. Defaults are also omitted from the encoded payload, so a plain bouquet's link stays as short as it was. **Verified both ways:** a hand-built pre-feature payload still opens, and a modern one restores the pigeon and the border.
- **The vessel indexes are stored in gift links — never reorder `vessels` or `prints`.** Same rule as the flower catalogue.

- A free, no-account digital bouquet: pick blooms, arrange them on a canvas, write a note, then share a link or download a PNG. `model.ts` holds the data + all canvas drawing, `BouquetStudio.tsx` the editor, `photos.ts` the image pipeline.
- **The gift travels in the URL fragment** (`#gift=<base64>`), so there is no database row and no upload. It is encoded, *not* encrypted — anyone with the full link reads the note. `validateBouquet` is the single gate: every bound it enforces is also the clamp the editor drags against, so a dragged piece can never land somewhere a decoded link would reject.
- **Photos finished 2026-09-12.** Up to 8, each re-encoded to WebP ≤640px (which also strips the original file's metadata), with 7 frames, free positioning, zoom/crop, front-or-behind layering, and a flood-fill background remover for cutouts. Found half-built: `photos.ts` and the `model.ts` half existed, `BouquetStudio.tsx` had **zero** references to any of it, so the feature was invisible. Now wired up as step 2 of 4.
- ⚠️ **Photo bouquets have no gift link, by design.** A data URL cannot ride in a URL fragment, so `encodeBouquet` refuses them and `decodeBouquet` rejects any link carrying photos. The Send step disables Copy/Share and points at Download instead. If photo links are ever wanted they need server-side storage, which does not exist.
- ⚠️ **`MAX_PHOTOS * MAX_PHOTO_LENGTH` is the total-size gate in `validateBouquet`.** It was hardcoded to 1,200,000 while the editor could produce 1,440,000 — a full bouquet validated fine in the editor and was then silently dropped on reload, losing the draft. Keep the two tied together.
- ✅ **All 54 flowers and the layered wrapper landed 2026-09-12.** The user supplied `wrapper-layers.webp` (4×2) and `flowers-a|b|c|d.webp` (4×3, 12 each) — converted PNG→WebP at q82, alpha kept. `pendingFlowers` is gone; every index 6–53 is pickable. Dropping below q82 saves ~13% because the weight is botanical detail, not compression, so it is not worth the quality.
- **The wrapper is a back/front pair per colour, which is the whole point.** `renderBouquet` draws the back half → photos/stems → the front half, so the flowers sit *inside* the paper instead of behind it. Pairs are `wrapIndex + 0 / + 1`, so a colour must stay on an even cell: **never insert a single cell into that sheet.** paper 0→ivory, 1→lilac, 3→black ("Midnight"), 4→pink ("Blush").
- ⚠️ **`renderBouquet` skips a stem whose sheet is missing, silently.** That is what made 48 blooms add an invisible stem before the art existed. A sheet that now 404s takes its own flowers out of the picker and offers a retry, rather than repeating that failure quietly.
- ⚠️ The picker fetches all five sheets (~2.4 MB) on /bouquet, because 54 CSS background-images all paint at once. Fine for a creative tool and cached after the first visit, but it is the page's weight — do not add sheets casually.
- **Flowers tilt by hand as well as by slider (2026-09-12).** A selected stem grows a dial beside its bloom; dragging it turns the flower about its base. It sits *perpendicular* to the stem, not past the tip — a tall stem puts its bloom near the top of the card, and anything further up lands outside the clip as an invisible handle. The grab offset is recorded on pointerdown so the flower turns *with* the finger instead of snapping under it, and the result is clamped to ±45° to match `validateBouquet`. The rotation slider is untouched.
- The "Midnight" paper's `sprite` is 7 (lavender wrapping); it was 4, the lavender *bloom*, which drew a giant flower where the wrapping goes. `sprite` now only matters if `wrapper-layers.webp` fails to load — the layered path indexes off the paper.
...
- **Domain: `mydayflower.com`, bought 2026-09-05 at Cloudflare Registrar** (ns `wally`/`wesley.ns.cloudflare.com`, expires 2027-09-05). Chosen over `dayflower.com`, which is real but listed at **$90,000** by a domain broker, and over `.co`/`.love`, whose renewals are 2–3× a `.com`. **Live on Vercel.** ⚠️ **Production deploys from the `calls` branch, not `main`** — `origin/main` is still at the PR #1 merge and has no `website/app/bouquet`, `/journal`, `/photobooth` or `/gifts` at all. Pushing `calls` is what publishes the site; merging to `main` publishes nothing. That is a workaround, and a trap: when `calls` is eventually merged and deleted, production silently loses its source. Move Vercel's Production Branch to `main` at the same time. Root Directory is **`website`** (repo root is the Flutter app), and `SUPABASE_URL` + `SUPABASE_ANON_KEY` must be set in the Vercel project or the waitlist route answers 503. Point DNS at Vercel **DNS-only (grey cloud)** — proxying puts two CDNs in series and breaks TLS issuance.

...
- 🔴 **48 of the 54 flowers and the layered wrapper are still waiting on artwork.** `flowers-a|b|c|d.webp` and `wrapper-layers.webp` are referenced by `model.ts` but **do not exist**, and `renderBouquet` skips a missing sprite sheet *silently* — so those blooms added an invisible stem. The names are parked in `pendingFlowers`; move a batch back into `flowers` only as a whole sheet lands (12 cells, 4×3), and **never reorder what is already live** — gift links store the flower index. `botanical-sprites.webp` is the only sheet that exists (4×2, see `public/bouquet/README.md`).
- The "Midnight" paper pointed at sprite 4, which is the lavender *bloom*, so it drew a giant flower where the wrapping goes. Now 7 (the lavender wrapping). `sprite` is only the fallback for when `wrapper-layers.webp` is absent; the layered path indexes off the paper instead.

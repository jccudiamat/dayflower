...
- Tests: test/screenshot_report_test.dart (Report attaches, none while writing, Not now, burst, switched off). Kotlin compiles (debug build). **Not tried on a phone**: screenshot events cannot be raised in a widget test, and the emulator would need a signed-in account.

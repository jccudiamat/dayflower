...
elsewhere. So does one whose strip is not plain paper: on an illustrated
frame, under the photo is often part of the drawing.
...
# Alpha from which a pixel counts as drawn, for the box round what a frame
# actually draws. Lower is the faint edge of a shadow or WebP's noise.
DRAWN = 32
...
# Share of a caption strip that must be plain paper: one colour, give or
# take the paper's grain. Under the photo on an illustrated frame is often a
# drawing (a capybara in a bath, a sleeping cat), and the words would be
# written across it.
PLAIN = 0.9

# How far from the strip's usual colour a pixel may be and still count as
# plain paper (a distance in RGB, out of 441).
PLAIN_REACH = 40.0
...
def plain(rgb, w, x0, x1, top_at, bottom_at):
    """Whether the band between two lines, over x0..x1, is plain paper."""
    samples = []
    for x in range(int(x0), int(x1) + 1, 3):
        for y in range(int(top_at(x)) + 1, int(bottom_at(x)), 3):
            i = (y * w + x) * 3
            samples.append((rgb[i], rgb[i + 1], rgb[i + 2]))
    if not samples:
        return False
    mid = tuple(sorted(c[k] for c in samples)[len(samples) // 2]
                for k in range(3))
    near = sum(1 for c in samples
               if sum((c[k] - mid[k]) ** 2 for k in range(3)) ** .5
               <= PLAIN_REACH)
    return near / len(samples) >= PLAIN
...
def caption_strip(alpha, rgb, labels, kept, w, h):
    ...
    for _, lab in kept:
        ...
        if tall < MIN_STRIP * h:
            continue
        # The part the words would cover, with its air trimmed off the same
        # way the spot is below.
        pad = (c - a) * (1 - STRIP_FILL[1]) / 2
        if not plain(rgb, w, runs[0][0], runs[-1][0],
                     lambda x: a + pad + slope * x,
                     lambda x: c - pad + slope * x):
            continue
...
def frame_windows(name):
    ...
    rgb = load_rgb(os.path.join(SRC, name + '.webp'))
    ...
    for n, lab in kept:
        ...
        windows.append(dict(
            ...
            points=pts))
    drawn = Image.frombytes('L', (w, h), alpha).point(
        lambda a: 255 if a >= DRAWN else 0).getbbox()
    content = (drawn[0] / w, drawn[1] / h,
               (drawn[2] - drawn[0]) / w, (drawn[3] - drawn[1]) / h)
    return windows, (w, h), caption_strip(alpha, rgb, labels, kept, w, h), content
...
CONTENT_HEADER = '''
// The box round what each frame actually draws, paper, tape and stickers,
// as fractions of its canvas. The canvas has clear margins round a tilted
// sheet, and Home fits this box rather than the canvas, or the paper comes
// out smaller than the space it was given.
const frameContent = <String, Rect>{
'''
...
def main():
    ...
    strips, contents = {}, {}
    for name in names:
        windows, (w, h), strips[name], contents[name] = frame_windows(name)
    ...
    out.append('};\n')
    out.append(CONTENT_HEADER)
    for name in names:
        l, t, cw, ch = contents[name]
        print('%-20s draws %.0f%% x %.0f%% of its canvas' % (
            name, cw * 100, ch * 100))
        out.append("  '%s': Rect.fromLTWH(%s, %s, %s, %s),\n"
                   % (name, fmt(l), fmt(t), fmt(cw), fmt(ch)))
    out.append('};\n')

...
export default function SiteHeader({ photobooth = false }: { photobooth?: boolean }) {
  return <header className="site-header"><nav aria-label="Main navigation" className="site-nav">
    ...
    <div className="site-nav-secondary"><Link href="/journal">Journal</Link><Link href="/#waitlist">Join the waitlist</Link></div>
    ...
    <details className="site-mobile-menu"><summary aria-label="More navigation"><span aria-hidden="true">☰</span></summary><div className="site-menu-panel"><Link href="/#waitlist">Join the app waitlist</Link><Link href="/journal">Read the Journal</Link></div></details>
  </nav></header>;
}

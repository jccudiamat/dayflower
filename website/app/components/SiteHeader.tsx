...
export default function SiteHeader({ photobooth = false, bouquet = false }: { photobooth?: boolean; bouquet?: boolean }) {
  return <header className="site-header"><nav aria-label="Main navigation" className="site-nav">
    ...
    <div className="site-nav-secondary"><Link href="/bouquet" aria-current={bouquet ? "page" : undefined}>Bouquet</Link><Link href="/photobooth" aria-current={photobooth ? "page" : undefined}>Photo booth</Link><Link href="/gifts">Gifts</Link><Link href="/journal">Journal</Link></div>
